...
def get_job_and_recommendations(job_id):
    try:
        ...
        # 비슷한 공고 검색 (sector와 location 기준)
        similar_jobs_query = """
        SELECT id, title, location AS region FROM jobs
        WHERE id != %s
        """
        filters = []
        params = [job_id]
        ...
        similar_jobs_query += " ORDER BY STR_TO_DATE(REPLACE(deadline, '~ ', ''), '%m/%d(%a)') ASC LIMIT 3"

        # 디버깅 로그
        print("Generated Query:", similar_jobs_query)
        print("Query Parameters:", params)

        cursor.execute(similar_jobs_query, params)
