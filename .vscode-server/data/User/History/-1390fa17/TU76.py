...
def add_bookmark():
    ...
    conn = mysql.connect()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO bookmarks (user_id, url, description, created_at) VALUES (%s, %s, %s, %s)", 
                   (user_id, url, description, datetime.utcnow()))
    conn.commit()
    cursor.close()
    conn.close()

    return jsonify({"msg": "Bookmark added successfully"}), 201
